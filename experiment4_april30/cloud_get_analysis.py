...
# the ith element is about run i (the ith time diploma server restarts, and 
#                                 the cloud server would be a second off)

cloud_get_latency = [[], [], [], [], []]
cloud_getNum = [0, 0, 0, 0, 0]
cloud_getGood = [0, 0, 0, 0, 0]
cloud_getGood_nonleader = [0, 0, 0, 0, 0]
cloud_getBad = [0, 0, 0, 0, 0] 
...
def dirWalk(dirname):
    ...
    global cloud_getGood_nonleader
    global cloud_getBad
        ...
        for filename in files:
            ...
            for line in open(os.path.join(path, filename)):
                ...
                if isGetValidR:
                    ...
                    g = re.search("CameraCloud Execute HTTP Download latency: (.*)ms", line)
                    if g is not None:
...
def printResults():
    for iRun in range(len(RUNTIMES)-1):
        ...
        if cloud_getNum[iRun] > 0:
            ...
            print "fails:\t\t%d\tdue to null region, but still with reply" % cloud_getBad[iRun]
...
if __name__ == "__main__":
    print "#############################################"
    print "###### Cloud Gets based on run number #####"
    print "#############################################"
