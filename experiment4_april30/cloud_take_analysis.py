        ...
        for filename in files:
            ...
            for line in open(os.path.join(path, filename)):
                ...
                if isLatencyOk:
                    ...
                    #sgood = re.search("(.*):.*one more takeGood", line)
...
def printResults():
    for iRun in range(len(RUNTIMES)-1):
        ...
        if cloud_takeNum[iRun] > 0:
            print "successes:\t%d\t%d %%\tincluding existing regions without a photo" % (cloud_takeGood[iRun], (cloud_takeGood[iRun]*100/cloud_takeNum[iRun]))
            print "fails:\t\t%d\tdue to null region, but still with reply" % cloud_takeBad[iRun]
