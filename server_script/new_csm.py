...
def take_leadership(x, y, id, modified_time):
        ...
        print '\ntake_leadership() in ('+str(x)+', '+str(y)+')'
        ...
        if regions[(x,y)]['leader'] is not None and regions[(x,y)]['leader'] == id:
                print 'take_leadership of the same phone for same region again'
                response = regions[(x,y)]
                response['status'] = CR_NOCSM
                return jsonify(response)

        # currently being led, by a different node, who checked in 130 secs or 2 minutes and 10 seconds ago ?
        if regions[(x,y)]['leader'] is not None and not regions[(x,y)]['leader'] == id and regions[(x,y)]['modified_time'] + 130000 > modified_time: 
                print 'take_leadership error'
...
def release_leadership(x, y, id, modified_time):
        ...
        print '\nrelease_leadership() in ('+str(x)+', '+str(y)+')'
